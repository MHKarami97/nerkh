...
async function main() {
  ...
  if (!items.length) { console.error('[fetch-dried-fruits] debug first 50 lines:', JSON.stringify(lines.slice(0, 50))); throw new Error('Could not parse any non-zero dried-fruits records') }
  const payload = { source: SOURCE_URL, generatedAt: new Date().toISOString(), sourceUpdatedAt, category: 'میوه خشک', currency: 'تومان', items }
