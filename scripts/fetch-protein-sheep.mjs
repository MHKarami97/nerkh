...
async function main() {
  ...
  if (rows.length < 1) throw new Error(`Could not parse any sheep records`)
  const payload = { source: SOURCE_URL, generatedAt: new Date().toISOString(), sourceUpdatedAt, category: 'پروتئین - گوسفند', currency: 'تومان', items: rows }
  await mkdir(dirname(OUTPUT_PATH), { recursive: true })
  await writeFile(OUTPUT_PATH, JSON.stringify(payload, null, 2) + '\n', 'utf8')
  console.log(`[fetch-protein-sheep] wrote ${rows.length} records`)
}
