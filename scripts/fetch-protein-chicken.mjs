...
function parseRow(row) {
  ...
  const priceCell = cells.find(cell => /تومان/.test(cell) && /[\d۰-۹]/.test(cell))
  const price = toNumber((priceCell || '').match(/[\d٬,]+/)?.[0])
  if (!price || price <= 0) return null
  const title = cells.find(cell => cell && cell !== priceCell && !/تومان|کیلو|گرم|تازه|داخلی|وارداتی|سرد|گرمسیری|سردسیری/i.test(cell)) || cells[0]
  if (!title) return null
  return { title, freshness: cells.find(cell => /تازه|منجمد|سرد/.test(cell)) || null, origin: cells.find(cell => /داخلی|وارداتی/.test(cell)) || null, unit: cells.find(cell => /کیلو|گرم/.test(cell)) || null, price }
}
