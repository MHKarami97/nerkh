...
async function main() { const html = await renderPage(); const sourceUpdatedAt = text(html).match(/([۰-۹\d]{1,2}\s+[\u0600-\u06FF]+\s+[۰-۹\d]{4})/)?.[1] || null; const items = html.split(/<tr[^>]*>/).map(parseRow).filter(Boolean); if (!items.length) throw new Error('Could not parse any non-zero veal records'); const payload = { source: SOURCE_URL, generatedAt: new Date().toISOString(), sourceUpdatedAt, category: 'پروتئین - گوساله', currency: 'تومان', items }; await mkdir(dirname(OUTPUT_PATH), { recursive: true }); await writeFile(OUTPUT_PATH, JSON.stringify(payload, null, 2) + '\n'); console.log(`[fetch-protein-veal] wrote ${items.length} records`) }
