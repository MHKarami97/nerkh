...
import { getProteinPrices } from '../services/foodProteinService.js'
...
onMounted(async () => {
  try {
    payload.value = await getProteinPrices()
  } catch {
    error.value = true
  } finally {
...
<template>
  <section class="food-section">
    <h2 class="section-title">مواد پروتئینی</h2>
    <p v-if="isLoading" class="food-section__state">در حال دریافت قیمت‌ها…</p>
    <p v-else-if="error" class="food-section__state">دریافت قیمت پروتئین ناموفق بود.</p>
    ...
    <template v-else>
      <div class="food-grid">
        <article v-for="item in visibleItems" :key="item.title" class="food-card">
          <div class="food-card__head"><strong>{{ item.title }}</strong><span v-if="item.category" class="food-card__cat">{{ item.category }}</span></div>
...
.food-card__cat { color:var(--text-muted); font-size:.7rem; white-space:nowrap; }
