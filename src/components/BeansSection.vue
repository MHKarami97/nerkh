...
import { getBeansPrices } from '../services/foodBeansService.js'
...
onMounted(async () => {
  try {
    payload.value = await getBeansPrices()
  } catch {
    error.value = true
  } finally {
...
<template>
  <section class="food-section">
    <h2 class="section-title">حبوبات</h2>
    <p v-if="isLoading" class="food-section__state">در حال دریافت قیمت‌ها…</p>
    <p v-else-if="error" class="food-section__state">دریافت قیمت حبوبات ناموفق بود.</p>
